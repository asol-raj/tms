import { addColumnBorders, advanceMysqlQuery, createFlyoutMenu, createFormAdvance, createFormSmart, createTable, fd2obj, fetchData, hideTableColumns, initAdvancedTable, inlineEditBox, jq, log, postData, setTableColumnWidths, titleCaseTableHeaders, toTitleCase, unHideTableColumns } from './help.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    jq('button.viewReport').on('click', async () => {
        try {
            ...
            let Year = jq('#sYears').val();
            let Month = jq('#sMonth').val();
            if (Year && Month) {
            ...
            const res = await postData(uri, { startOfMonth, userId }); log(res)
            const rows = res?.data || [];
            if (!rows.length) {
...
async function loadData(url = '/auth/daily/tasks/data') {
    try {
        ...
        const tasks = res?.tasks ?? []; log(tasks); //return;
        ...
        $thead.find('tr').append('<th data-key="remarks">Remarks</th>');
        $tbody.find('tr').append('<td data-key="remarks"></td>');

        $thead.find('tr').append('<th data-key="comments">Comments</th>');
        $tbody.find('tr').append('<td data-key="comments"></td>');
        ...
        hideTableColumns($table, ['task_list_id', 'assignment_id', 'assignment_start_date', 'assignment_end_date', 'completion_id', 'assignment_active', 'created_at', 'updated_at', 'task_active', 'assigned_on', 'remarks', 'comments', 'comment_by_id']);
        ...
        // generic helper to wire inline edit for a column
        function attachRemarksEditor(columnName, buildExtraPayload) {
            ...
            inlineEditBox($tbody, columnName, (value, cell, $row) => {
                ...
                axios.post(url, payload, { withCredentials: true })
                    ...
                    .finally(() => {
                        // any role-specific cleanup can go here if needed
                    });
            });
        }

        // Now plug in user vs admin behavior

        if (role === 'user') {
            // user can only edit "remarks" for himself
            attachRemarksEditor('remarks', (value /*, $row*/) => ({
                remarks: value
            }));
        } else {
            // admin can edit "comments" for a selected user
            attachRemarksEditor('comments', (value /*, $row*/) => ({
                comments: value,
         ...
         if (role === 'user') {
            ...
            const selectuser = $userSelect.val();
            if (selectuser) {
                unHideTableColumns($table, ['remarks', 'comments']);
            } else {
...
async function loadYears() {
    try {
        ...
        const option = new Option('Select Year', '');
        $selectYears.append(option);

        const res = await advanceMysqlQuery({ key: 'na', qry: 'SELECT DISTINCT YEAR(created_at) years FROM tasks_list ORDER BY years;' });
        const arr = res?.data || [];
        if (arr.length > 0) {
            arr.forEach(y => {
                let years = new Option(y.years);
                $selectYears.append(years);
            })
        }
        loadMonths()
    } catch (error) {
